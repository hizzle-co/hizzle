				...
				{ ( { onClose } ) => (
					<VStack>
						...
						{ hasResults && (
							<>
								<Tip>
									{ __( 'Click a field to insert it.', 'newsletter-optin-box' ) }&nbsp;
									{ __( 'When your automation runs, each field will be replaced with actual data.', 'newsletter-optin-box' ) }
								</Tip>
								{
									Object.keys( filteredGroups ).map( ( group ) => (
										...
										>
											{ filteredGroups[ group ].map( ( item ) => {
												...
												return (
													<MenuItem
														icon={ isPremium ? lock : ( item.icon || next ) }
														iconPosition="left"
						...
						{ !hasResults && (
							<MenuGroup>
								<MenuItem disabled>
									{ __( 'No matching items found', 'newsletter-optin-box' ) }
								</MenuItem>
							</MenuGroup>
						) }
					</VStack>
