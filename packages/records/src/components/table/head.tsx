...
import { arrowLeft, arrowRight, arrowUp, arrowDown, unseen } from '@wordpress/icons'
...
const HeaderMenuToggle = memo(
    forwardRef(
        function HeaderMenuToggle( { children, ...props }, ref ) {
            return (
                <Button
                    { ...props }
                    size="compact"
    ...
    return (
        <thead>
            { table.getHeaderGroups().map( headerGroup => {
                ...
                return (
                    <tr key={ headerGroup.id } className="hizzle-records__table__row">
                                                ...
                                                <>
                                                    ...
                                                    { ( header.column.getCanSort() && header.column.getIsSorted() ) && (
                                                        <span aria-hidden="true">
                                                            { sortArrows[ header.column.getIsSorted() as keyof typeof sortArrows ] }
                                                        </span>
                                                    ) }
                                                </>
                                            ),
                                        ...
                                        { ( { onClose } ) => {
                                            ...
                                            return (
                                                <>
                                                    { header.column.getCanSort() && (
                                                        <MenuGroup label="Sort">
                                                            { SORTING_DIRECTIONS.map( direction => (
                                                                <MenuItem
                                                                    icon={ sortIcons[ direction as keyof typeof sortIcons ] }
                                                                    key={ direction }
                                                                    onClick={ () => {
                                                                        // Close the menu.
                                                                        onClose();
                                                    ...
                                                    { ( canMove || header.column.getCanHide() ) && (
                                                        <MenuGroup>
                                                            { canMove && (
                                                                <MenuItem
                                                                    icon={ arrowLeft }
